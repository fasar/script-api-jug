...
class UserResTest {
    ...
    @Test
    fun readUserTest() {
        val user = mapper.readValue<UserRes>(user, UserRes::class.java)
    ...
    fun readUserInReaderTest() {
        val userReader = StringReader(user)
        val user = mapper.readValue<UserRes>(userReader, UserRes::class.java)
        Assert.assertNotNull(user)
        println("User is : $user")
    }
}